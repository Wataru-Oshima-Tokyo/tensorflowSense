...
class Img_Retriever():
    def __init__(self, tfFolder):
        self.MODEL_NAME = tfFolder
        ...
        self.sub = rospy.Subscriber('/camera/color/image_raw', Image, self.img_callback)
    ...
    def img_callback(self,ros_pics):
        try: 
            t1 = cv.getTickCount()
...
def main():
    ...
    try:
        tfFolder = rospy.get_param("~modeldir")
        print(tfFolder)
    except:
        tfFolder =""
        print("could not get param name")
    # startup()
    Img=Img_Retriever(tfFolder)
    rospy.spin()
